...
server/            # API
  _core/           # handler.ts, context.ts, trpc.ts, env.ts,
                   # sdk.ts（JWT セッション）, cookies.ts, index.ts（Node）, vite.ts, storageProxy.ts
...
- クライアントは `credentials: "include"` でセッションクッキーを送受信する（GH Pages と Worker の別オリジン対応）
...
- **v1.2.0**: デモモード（`VITE_DEMO_MODE`、認証バイパスで全機能確認可能、ローカル開発専用）。公開ランディングページを `/` に新設し dashboards を `/dashboard` 以下へ移動（`App.tsx` に `Route "/"` = `LandingPage`）。LP のみでは操作体験できない旨を README に明記。ユーザー名+パスワードログインの PBKDF2 を Cloudflare Workers 制約に適合（bypass→`HASH_ALG` を `"SHA-256"` の正規名に、反復回数を 100,000 に）（`server/password.test.ts` を新設、計 31 テスト）。本番 Worker へ D1 migration `0001` 適用とデプロイを実施し、実 API で login/me/logout/保護ルート/ロール制御を検証。README(ja/en)・DEV-MEMO の記述を現在の実装・テスト数に一致させた。package.json / バッジを v1.2.0 に更新
